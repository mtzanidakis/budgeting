...
const (
	upgradeRepo        = "mtzanidakis/budgeting"
	upgradeBinary      = "budgeting-cli"
...
func runUpgrade(args []string) {
	...
	targetVer := fs.String("version", "", "install this exact version instead of latest (e.g. v0.3.3)")
	yes := fs.Bool("yes", false, "confirm in-place replacement of this binary")
	_ = fs.Parse(args)

	if runtime.GOOS == "windows" {
		fail("upgrade is not supported on windows; download a fresh archive from https://github.com/%s/releases", upgradeRepo)
	}
	...
	if target == "" {
		...
		if err != nil {
			fail("fetch latest release: %v", err)
		}
		target = latest
	}
	...
	if current == "dev" {
		fail("cannot upgrade a dev build; install a released binary first (https://github.com/%s/releases)", upgradeRepo)
	}
	if current == target {
		fmt.Printf("%s %s already up to date\n", upgradeBinary, current)
		return
	}
	if !*yes {
		fail("would upgrade %s → %s; pass --yes to proceed", current, target)
	}

	bareVer := strings.TrimPrefix(target, "v")
	asset := archiveName(upgradeBinary, bareVer, runtime.GOOS, runtime.GOARCH)
	...
	binBytes, err := extractBinary(archiveBytes, upgradeBinary)
	if err != nil {
		fail("extract: %v", err)
	}
	if err := replaceSelf(binBytes); err != nil {
		fail("replace binary: %v", err)
	}
